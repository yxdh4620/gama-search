...
(function() {
  var GET_HTTP_METHOD, POST_HTTP_METHOD, SEARCH_LIST_SIZE, SIGN_MODE, SearchManager, assert, cryptoUtil, path, request, urlEncode, urlUtil, _, _makeFields, _makeFilter;
  ...
  SearchManager = (function() {
    ...
    SearchManager.prototype.search = function(queryStr, fieldName, filter, page, callback) {
      ...
      params['query'] = "config=fromat:json,start:" + start + ",hit:" + this.pageSize + "&&query=" + fieldName + ":'" + queryStr + "'&&filter=" + (_makeFilter(filter));
      params['index_name'] = this.indexName;
      params['Signature'] = cryptoUtil.makeSign(params, GET_HTTP_METHOD, this.accessKeySecret);
      options = {
        url: "" + url + "?" + (urlEncode.query2string(params)),
      ...
      request(options, (function(_this) {
        return function(err, res, body) {
          if (!err) {
            body = _this._parseResult(body, page);
          }
          callback(err, body);
        };
      })(this));
    ...
    SearchManager.prototype.advancedSearch = function(queryStr, fieldName, page, subQuerys, others, callback) {
      ...
      subQuerys = subQuerys || {};
      others = others || {};
      query = "config=fromat:json,start:" + start + ",hit:" + this.pageSize + "&&query=" + fieldName + ":'" + queryStr + "'";
    ...
    SearchManager.prototype._parseResult = function(data, page) {
      var result;
      if (!data) {
        return data;
      }
      ...
      result['page'] = page;
      result['pagetotal'] = parseInt((result.viewtotal + this.pageSize - 1) / this.pageSize);
      data.result = result;
      return data;
    };
